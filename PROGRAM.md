...
## §82 — Proactive OOM defense: defer idle work near the container memory ceiling (2026-05-31)

Closes the **secondary** cause flagged in §81's diagnosis. §81 fixed the slow-boot watchdog loop and
made the watchdog *report* `OOMKilled`/exit 137 after the fact; §82 adds the proactive half — backing off
idle work *before* the gateway hits its 8 GB Docker memory limit (`docker-compose.yml`
`deploy.resources.limits.memory: 8g`). An OOM SIGKILLs the gateway; `restart: unless-stopped` then brings
it back cold, which can re-enter the §81 watchdog window — so reducing OOM pressure directly cuts restart
floods.

**Gap.** The idle scheduler already defers MEDIUM/HEAVY jobs under resource pressure via
`app/substrate/policy.py:should_defer_heavy_work` (disk free, inflight tasks, Q16 `host_substrate` alerts).
But none of those track the gateway's **own cgroup memory** against its limit. On macOS Docker the
in-container `/proc/meminfo` reflects the Linux VM, not the 8 GB cgroup cap, so the existing
`low_memory_headroom` signal can't see the gateway approaching the ceiling the OOM killer actually enforces.

**Fix (additive, fail-open).** New `app/substrate/status.py:_read_cgroup_memory()` reads the limit the OOM
killer uses directly — cgroup v2 (`/sys/fs/cgroup/memory.current` + `memory.max`) with a cgroup v1 fallback
(`memory.usage_in_bytes` + `memory.limit_in_bytes`); returns `(used, None)` when no limit is set
(`memory.max == "max"` / the v1 ~2**63 sentinel) and `(None, None)` off-Linux. `_probe_resources` surfaces
`cgroup_mem_used_fraction` / `_used_mb` / `_limit_mb` on the snapshot. New `ResourcePolicy.max_cgroup_mem_fraction`
(default 0.90) makes `should_defer_heavy_work` return a defer reason when usage ≥ 90 % of the cgroup limit —
so the next MEDIUM/HEAVY idle job publishes an `idle_job_deferred` event (no silent skip) instead of pushing
the container into OOM. `None` fraction ⇒ predicate skipped (non-Linux / no limit), preserving prior behaviour
exactly. LIGHT jobs are never deferred (unchanged).

**Verification.** 9 tests in `tests/test_substrate_cgroup_mem.py` pin the predicate (defer ≥ threshold, run
below, skip when absent, tunable threshold, boundary), the reader's never-raises + no-limit handling, and the
probe populating the fraction. The reader validated live against this very container's cgroup v2 (read
`memory.current` ≈ 722 MB, `memory.max == "max"` → correctly reported no-limit). No TIER_IMMUTABLE touches, no
...
## §85 — Script-exec hardening + dead-façade/orphan-store sweep (2026-06-05)

Two threads from the whole-project code review, landed together (PR #148).

**Thread 1 — harden the in-process `exec` of agent-authored scripts.** `app/tools/pdf_compose.py` and `app/tools/gee_tool.py` run LLM-generated Python in-process via `exec(script, namespace)`. With a plain namespace Python injects the **full** `__builtins__`, so a prompt-injected or simply-erroneous script could `import os; os.system(...)`, `import socket`/`requests` to exfiltrate, or `open('/proc/self/environ')` / read a token file to lift the gateway's secrets. New `app/tools/_safe_exec.py` builds a restricted `__builtins__` and `harden(namespace)` installs it immediately before each `exec`:
- removes the dynamic-exec builtins (`eval`/`exec`/`compile`/`breakpoint`/`input`/`__import__`/`open` — the last two replaced by guarded versions);
- guarded `__import__` REFUSES egress / process-spawn / introspection-escape modules (`subprocess`, `socket`, `ssl`, `requests`, `httpx`, `ctypes`, `multiprocessing`, `sys`, `importlib`, `pickle`, `marshal`, `gc`, …) and hands back a safe `os` **shim** (`os.path` + benign FS helpers; no `system`/`popen`/`exec*`/`environ`/`fork`);
- guarded `open` denies `/proc`·`/sys`·`/dev` prefixes and secret-looking paths (`token`/`secret`/`credential`/`password`/`.env`/`.key`/`.pem`/`id_rsa`).

Design intent preserved — report/geo scripts still `import` their data/plotting/geo libs, use `os.path`, `open` workspace files, and call pre-bound helpers (`plt`/`np`/`ee`/`render_map`). **Honest caveat (in the module docstring):** this is a proportionate barrier against the realistic threat (an LLM emitting `import os`/`import socket`), NOT a hardened sandbox — a *crafted* payload can still reach an already-imported dangerous module via object-subclass traversal. Complete isolation needs a separate container with no secret env/mounts, which is incompatible with these tools' needs (gee requires network+auth; pdf writes the shared output dir), so it is deliberately not pursued here; use as defence-in-depth. 28 tests in `tests/test_safe_exec.py`.

**Thread 2 — remove 8 dead re-export façades + orphaned stores (~933 lines).** Focused orphan/façade sweep following the review. All targets verified **OPEN tier** with zero static/dynamic importers in `app/`:
- **Pure re-export façades** (0 `def`/`class`, never adopted — every consumer imported the real modules directly): `app/adaptation/`, `app/request/`, `app/llm_suite/`, `app/operations_suite/`, `app/control/` (its apparent "129 importers" were all `control_plane` substring matches — real count 0), `app/lazy_imports.py`.
- **Production-orphaned stores** (only test / boot-seed references remained): `app/contracts/` (Firestore schema; firebase is vestigial-by-default) and `app/evolution_db/` (`eval_sets` boot-seeded an **unread** postgres table; `judge` had no caller — both superseded by `eval_primitives` + `worktree_eval` in the §73 verified-engine consolidation).

Reference cleanup: dropped the `evolution_db` boot seed in `main.py` and the seven test references that pinned the dead modules (`test_knowledge_base`, `test_phase0_plumbing`, `test_philosophy`, `test_subsystem_wiring`). `app/worker.py` was correctly **excluded** as a false-positive (0 importers, but it is the deliberate `python -m app.worker` entrypoint for the in-flight serving-split `worker` compose service). `docs/ARCHITECTURE.md` "Package Boundaries" rewritten to reflect direct-import reality.

No TIER_IMMUTABLE touches, no Tier-3 amendments, no new master switches. 47 stdlib tests pass (`test_safe_exec` + `test_phase0_plumbing`); app-dependent suites collect clean; full-repo scan confirms zero dangling references. This pass is itself a counter to the recurring meta-pattern (the system accretes gated layers and rarely deletes) — worth a periodic sweep after each consolidation.

## §86 — Gateway serving-plane hardening: chroma named volume + loop sentinel + persisted cadence (2026-06-12)

Root-cause closure for the 2026-06 wedge class (memory `project_recurring_incidents_2026_06_07`, §77/§79–§83 contained symptoms): the asyncio event loop blocked on synchronous bind-mount I/O under heavy idle KB writes → `/health` dark >900s with all threads sleeping → host-watchdog restart → monotonic cadence reset made all ~76 gated jobs instantly due → catch-up herd → re-wedge (3 wedge-restarts on 2026-06-12 alone, even with the 06-08 LIGHT-pool-serialization override active — mitigation space was exhausted). Operator-approved plan, four phases + gated fifth; all live same day. See `docs/GATEWAY_SERVING_PLANE.md` (runbook + layout + rollback).

**Phase 0 — landed the unshipped fixes.** The 2026-06-08 fiction-ingest wedge fix (LIGHT→HEAVY + provisional per-book skip-marker so a SIGKILL can't re-loop ingestion; also discover-topics/self-model-refresh off LIGHT) had sat unmerged on `fix/idle-job-wedge-fiction-2026-06-08` while the running gateway wedged on exactly that job; merged. The same-day signal-cli-0.14.5 Host-header fix (`421 Misdirected Request` rejected every gateway send; `_http_session.headers["Host"]="localhost"` in `app/signal_client.py` — TIER_IMMUTABLE, operator-approved) existed only in the running image; committed. Also discovered: the §PR-133 push-to-main auto-deploy webhook was never actually installed on the host (plist exists in `scripts/`, not in launchd) — deploys are manual `docker compose build gateway && up -d --no-deps gateway`.

**Phase 1 — the physics fix: chroma → named volume.** ChromaDB is a derived, ledger-rebuildable index (§56) sitting on the worst I/O substrate (macOS bind mount, fsync amplification). New single choke point in `app/paths.py`: `CHROMA_DATA_ROOT` (env; default = `WORKSPACE_ROOT` identity so dev/tests are byte-identical), `chroma_kb_dir()`, `chroma_root()`, `chroma_split_active()` (env-keyed on purpose — robust against harnesses that monkeypatch `WORKSPACE_ROOT`). Routed: `chromadb_manager` (PERSIST_DIR + `get_kb_client`), all 7 per-KB configs (per-KB env overrides still win), `chromadb_rebuild` (data → volume; `.rebuild_backups` stay workspace-side), `business_store` (+ raw `PersistentClient` → cached `get_client_for_path`), `dr/export_kbs` discovery, `chromadb_integrity` (discovery + boot scan walk the chroma root; quarantine renames stay inside the volume; `daily_snapshot` redirects to `workspace/<kb>/.sqlite_snapshots` only when split active), `chromadb_hygiene`, `db_backup`. New **empty-volume boot guard**: chroma root empty + non-empty ledgers ⇒ loud Signal alert; recovery via the source-ledger daemon's next pass (never inline — lifespan stays fast per §81). Free-space visibility: substrate `chroma_disk_free_gb` + policy floor + disk_quota second probe (`disk_quota_chroma`). Compose: named volume `chroma_data` → `/chroma` on the gateway; **worker gets neither mount nor env** (§55 single-writer enforced physically; pinned). Migration executed in a watchdog-paused window via `scripts/migrate_chroma_to_volume.sh`: 7 KBs (~207 MB), per-KB `PRAGMA integrity_check` all ok, sqlite row counts match source exactly; bind-mount originals retained 7 days (rollback = unset env). Boot on volume: integrity scan all-7-ok, RAG stores pre-warm clean, `/health` in 10 s.

**Phase 2 — loop sentinel + serving-plane I/O hygiene.** New `app/loop_sentinel.py`: heartbeat task on the loop + monitor daemon thread; >5 s stall ⇒ `faulthandler` dump of EVERY thread's stack to `workspace/healing/loop_stalls/<ts>.txt` **while blocked** (each future wedge names its culprit line), stall count + duration + p95 lag via `get_stats()`. Wired in the `main.py` lifespan beside the liveness heartbeat (TIER_IMMUTABLE, operator-approved). The actual unblocking: the `crewai.audit` logger and the root structured-error handler were synchronous `RotatingFileHandler`s writing to the bind mount from the loop thread — both now stdlib `QueueHandler`/`QueueListener` (lock-free emit; one listener thread owns disk; rows carry their own ts so latency_slo pairing is unaffected); `signal_inbound`'s `enqueue_inbound` (sync SQLite commit) moved off-loop via `asyncio.to_thread`. **Congestion control**: substrate snapshot carries `loop_in_stall`/`loop_last_stall_age_s`/`loop_lag_p95_ms`; `should_defer_heavy_work` returns `event_loop_degraded` (in-stall / stalled <600 s ago / lag-p95 >500 ms) so MEDIUM/HEAVY idle work yields to a degraded serving plane. New healing monitor `loop_stall` (5-min cadence) Signal-alerts new stalls with the dump path.

**Phase 3 — restart-herd control.** Cadence gates converted from in-memory `time.monotonic()` (reset every restart ⇒ herd) to **wall-clock last-runs persisted** in the existing `idle_job_state` dbm (`last:<name>`, sibling of `skip:`/`fail:`); a daily job that ran 2 h ago stays not-due across restarts; clock-backwards steps rewrite the stamp once (a sliding-now wedge the new test caught in review). Deterministic crc32 jitter (≤600 s daily-scaled — NOT salted `hash()`) spreads genuine post-outage herds. **Cadence-map completion**: 31 MEDIUM/HEAVY jobs were absent from `_HEAVY_MIN_CADENCE` and ran every round-robin rotation — including `alignment-audit` (intended weekly; its every-cycle LLM runs are the known false-CRITICAL source, memory `project_alignment_audit_false_critical`) and the wedge trio; all mapped (alignment-audit weekly, autonomous-executor 15 min, fiction-ingest daily, …); stale `evolution` key removed (matched nothing since the job's rename). `tests/test_idle_cadence_invariants.py` pins: persistence across restart, due-jobs still run, jitter determinism/bounds, **full M/H coverage (CI fails on unmapped new jobs)**, no stale keys, wedge-trio never LIGHT, clock-skew never wedges.

**Phase 4 — worker re-enable (GATED, pending).** Precondition: 48 h with zero watchdog `Threshold breached` restarts AND zero loop-stall dumps. Then remove `docker-compose.override.yml` (06-08 LIGHT-serialization + SYS_PTRACE diagnostics — superseded) and `docker compose --profile worker up -d worker` (foundation complete: 17-job allowlist, fail-closed guard, kb_proxy). Rollback: stop worker.

**Verification at ship**: in-container suites green (`test_chroma_data_root` 36, sentinel+cadence+substrate+warmup 65; 1 pre-existing unrelated cgroup-test failure); `/health` SLO sample post-deploy p50 21 / p95 44 / **p99 63 ms**, 0 failures (warm-up window; heavy-load re-check belongs to the soak); latency_slo baseline re-establishes fresh. **Deferred by operator decision**: pg/neo4j named-volume move folds into the PG18 window. New surfaces: 1 healing monitor (`loop_stall`), 2 `ResourcePolicy` fields, `CHROMA_DATA_ROOT` env, `chroma_data` volume, `scripts/migrate_chroma_to_volume.sh`; no new master switches, no Tier-3 amendments; TIER_IMMUTABLE touches (`app/signal_client.py`, `app/main.py`, `docker-compose.yml`) operator-approved via the 2026-06-12 plan.
