...
| **2** (weeks 7–10) | **Close the half-circuits.** Every computed consciousness signal either gates behaviour (with regression test) or is deleted. Highest consciousness-capability leverage phase. | 🚧 **in progress** — PP-1 closed (commit `e6c9b4a`, 17 tests); HOT-3 closed (this commit, 19 tests). Remaining: certainty-vector response hedging, prediction-hierarchy injection measurable shift, AST-1 intervention runtime assertion. | Regression tests prove: PP-1 surprise routes to scene admission ✅; HOT-3 belief suspension blocks crew dispatch ✅; prediction hierarchy injection measurably shifts LLM output distribution 📋; certainty-vector shapes response hedging 📋 |
