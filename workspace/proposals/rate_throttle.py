...
class RequestCostTracker:
    ...
    def summary(self) -> dict:
        """Return a dict with the accumulated usage stats."""
        with self._lock:
            return {
...
def start_request_tracking(request_id: str = "") -> RequestCostTracker:
    """Start tracking LLM token usage for the current request."""
    tracker = RequestCostTracker(request_id)
    _request_cost.set(tracker)
    return tracker


def end_request_tracking() -> dict | None:
    """End tracking and return the final usage stats."""
    tracker = _request_cost.get(None)
    if tracker is None:
        return None
