...
#
# Every factory-managed Anthropic call goes through the same triad:
#   1. ``pre_check`` against the daily cap
#   2. Run the actual call
#   3. Observe the outcome (mark_alive on success, mark_dead on a
#      model-id-level 404, no-op on other exceptions)
#
# Before this helper, the triad was open-coded in 4 places
# (CreditAware.call / CreditAware.acall / Handle.create / Handle stream).
# A divergence in any one of those — say, a missing pre_check, or
# mark_alive being called on a 5xx — would silently bias the cache.
# Centralising here keeps the contract under one roof.


def observe_outcome(bare_model: str, *, exc: BaseException | None) -> None:
    """Update the health cache after an Anthropic call.

    Used by streaming paths (where ``mark_alive`` fires at iterator
    exhaustion, not at call open) and by call sites that want to
    manage their own pre-check / failover layering.  ``exc=None``
    means the call succeeded; any other value is classified — a
    model-id-level 404 marks dead, every other exception class is a
    no-op (it belongs to a different layer: rate-limit, credit
    breaker, network, etc.).
    """
    if exc is None:
        if bare_model:
            mark_alive("anthropic", bare_model)
        return
    reason = classify_failure(exc)
    if reason and bare_model:
        mark_dead("anthropic", bare_model, reason)


def call_with_observation(
    bare_model: str,
    call_fn,
    *,
    estimated_cost_usd: float = 0.0,
):
    """Run ``call_fn()`` under the full observation envelope.

    Steps:
      1. ``pre_check(estimated_cost_usd)`` — raises
         :class:`app.llm_anthropic_budget.AnthropicDailyCapExceeded`
         when the next call would push rolling-24h spend past the cap.
      2. ``call_fn()`` runs; result returned.
      3. On exception: outcome observed via :func:`observe_outcome`,
         then exception re-raised unchanged.
      4. On success: ``mark_alive``, result returned.

    The synchronous sibling of :func:`acall_with_observation`.  Used
    by :class:`app.llm_factory.AnthropicClientHandle` (non-streaming)
    and :class:`app.llms.credit_aware_anthropic.CreditAwareAnthropicCompletion`
    sync path.
    """
    ...
    try:
        ...
        observe_outcome(bare_model, exc=exc)
        raise
    observe_outcome(bare_model, exc=None)
    return result


async def acall_with_observation(
    bare_model: str,
    acall_fn,
    *,
    estimated_cost_usd: float = 0.0,
):
    """Async sibling of :func:`call_with_observation`.

    ``acall_fn`` is an awaitable factory: ``acall_fn()`` returns the
    coroutine to await.  Same observation envelope.
    """
    from app.llm_anthropic_budget import pre_check
    pre_check(estimated_cost_usd=estimated_cost_usd)
    try:
