...
class Settings(BaseSettings):
    ...
    # 2026-07-24: bumped 6 → 16, then REVERTED same day. The intent was
    # sound (see reports/ANSWER_QUALITY_DIAGNOSIS_2026-07-24.md — the pool
    # is shared by run_parallel's OUTER multi-crew dispatch AND every INNER
    # sub-agent/debate-round fan-out, so 6 workers let research sub-agents
    # queue behind an unrelated outer crew for its whole runtime), but the
    # bump had an unaccounted-for dependency: every agent construction
    # (``create_specialist_llm``) does a SYNCHRONOUS Postgres query for
    # benchmark scores (``llm_benchmarks.get_combined_scores`` ->
    # ``llm_external_ranks.get_external_score``) against a fixed 24-conn
    # pool (``CONTROL_PLANE_POOL_MAX``). More concurrent threads meant more
    # concurrent DB-pool contention, and a live eval-harness run the same
    # day produced the worst event-loop wedge in the project's history
    # (8 simultaneous psycopg2/pool.py frames in a loop-stall dump,
    # workspace/healing/loop_stalls/20260724T171644Z.txt) plus three
    # watchdog-triggered restarts in ~2.5h that kept recurring minutes
    # after each fresh boot. Reverted to the safe prior value the same day.
    # A real fix for the original starvation problem needs to ALSO account
    # for DB-pool capacity (e.g. cache benchmark scores, or size this
    # against CONTROL_PLANE_POOL_MAX), not just raise the thread ceiling —
    # left for a follow-up, not reattempted here under incident pressure.
    ...
    # 2026-07-24: bumped 2 → 8, then REVERTED same day alongside
    # thread_pool_size — see that field's comment for the full incident
    # account (DB-connection-pool contention from create_specialist_llm's
    # synchronous benchmark-score lookup, not this semaphore itself, was
    # the actual mechanism; caught by a live eval-harness run the same
    # day: reports/ANSWER_QUALITY_DIAGNOSIS_2026-07-24.md). Despite the
    # name, this semaphore (app/crews/parallel_runner.py) throttles EVERY
    # run_parallel task, not just Ollama-routed ones — a vestige of the
    # pre-consolidation local-first design, and a real fix for the
    # original nested-fan-out starvation problem needs to raise this
    # WITHOUT exceeding what the shared Postgres connection pool
    # (CONTROL_PLANE_POOL_MAX, default 24) can absorb — left for a
    # follow-up rather than reattempted here under incident pressure.
