...
def install_throttle() -> None:
    ...
    with _patch_lock:
        ...
        # (The former OpenAI-SDK credit-failover monkeypatch was removed in
        # the OpenRouter+Ollama consolidation. Every OpenRouter LLM is now
        # built with is_litellm=True — see app/llm_factory._cached_llm — so
        # all network traffic flows through litellm.completion and is covered
        # by the _throttled_completion wrapper above. CrewAI's native
        # openai-SDK provider branch is no longer reachable for our traffic.)
