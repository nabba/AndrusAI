---
sections:
  archibal: 0
  kaicart: 0
  meta: 74
  philosophy: 0
  plg: 0
  self: 3
title: AndrusAI Wiki — Master Index
total_pages: 77
updated_at: '2026-06-25T09:47:25Z'
---
# AndrusAI Knowledge Wiki — Master Index

Total pages: 77 | Last updated: 2026-06-25

## Meta (Cross-Venture)
- [[meta/1-1ca80e91]] — **1. **
- [[meta/1-topic-55570dd0]] — **1. Topic**
- [[meta/462f5f1b]] — ****
- [[meta/adaptive-search-depth-and-confidence-gating]] — Adaptive Search Depth and Confidence-Gated Research
- [[meta/advanced-policy-white-paper-critique-methodology-1a6d7ab0]] — Advanced policy white paper critique methodology
- [[meta/advanced-vector-database-optimization-retrieval-augmentation-ra-strategies-cd1d8]] — Advanced Vector Database Optimization & Retrieval Augmentation (RAG) Strategies
- [[meta/answer-completeness-verification]] — Answer Completeness Verification (Pre-Finalization Check)
- [[meta/api-credit-management-and-quota-error-handling]] — API Credit Management and Quota Error Handling
- [[meta/atomic-synthesis-and-concise-reporting]] — Atomic Synthesis and Concise Reporting Protocol
- [[meta/automated-pdf-report-generation-with-graphics-7cea4e8a]] — automated_pdf_report_generation_with_graphics
- [[meta/budget-report-retrieval-protocols-621666f0]] — Budget_report_retrieval_protocols
- [[meta/circular-import-resolution-patterns]] — Circular Import Resolution Patterns
- [[meta/create-yearly-deforestation-maps-2012-2025-bc2e815c]] — create_yearly_deforestation_maps_2012_2025
- [[meta/creating-a-safe-command-execution-wrapper-in-python-da30cc0c]] — **** Creating a Safe Command-Execution Wrapper in Python
- [[meta/distinguishing-authentication-and-authorization-28efaccb]] — **** Distinguishing Authentication and Authorization
- [[meta/email-inbox-retrieval-and-ranking-protocols-07e81df8]] — Email inbox retrieval and ranking protocols
- [[meta/email-ranking-accuracy-improvement-fda3766d]] — Email_ranking_accuracy_improvement
- [[meta/error-patterns-resolution]] — Error Patterns and Resolutions
- [[meta/eston-cc145a47]] — Eston
- [[meta/estonia-forest-maps]] — Skill: Generating Estonia Deforestation & Forest Age Maps via Hansen GFC
- [[meta/estonian-deforestation-policy-research-9116c2f7]] — Estonian deforestation policy research
- [[meta/estonian-document-translation-pipeline-eb7934f9]] — estonian_document_translation_pipeline
- [[meta/estonian-environmental-movement-nomenclature-analysis-000db5e6]] — Estonian environmental movement nomenclature analysis
- [[meta/estonian-environmental-policy-research-framework-7bae0331]] — Estonian environmental policy research framework
- [[meta/estonian-forest-protection-movement-analysis-5fccee21]] — Estonian forest protection movement analysis
- [[meta/estonian-forestry-statistical-data-sourcing-f13dd107]] — Estonian forestry statistical data sourcing
- [[meta/estonian-pdf-improvement-suggestion-workflow-0051d6a2]] — Estonian PDF improvement suggestion workflow
- [[meta/estonian-pdf-white-paper-analysis-bfe54d6a]] — Estonian PDF white paper analysis
- [[meta/estonian-policy-document-critique-15141d9e]] — Estonian policy document critique
- [[meta/forest-report-pdf-generation-74283fcd]] — forest report PDF generation
- [[meta/gee-batching-pattern]] — Google Earth Engine: One Round-Trip Per Script
- [[meta/gee-forest-metric-semantics]] — Forest metrics in GEE: pick the metric you actually need
- [[meta/google-earth-engine-scripting-estonian-forest-monitoring-4614fa6f]] — Google Earth Engine scripting Estonian forest monitoring
- [[meta/handle-task-circular-import-immediate-fix]] — Handling handle_task.py Circular Import Error
- [[meta/high-fidelity-evidence-mapping-and-triangulation]] — High-Fidelity Evidence Mapping and Triangulation
- [[meta/high-fidelity-synthesis-signal-formatting]] — High-Fidelity Synthesis & Signal-Optimized Formatting
- [[meta/identifying-regional-payment-service-providers-psps-with-direct-sales-con-51bccc]] — **** Identifying Regional Payment Service Providers (PSPs) with Direct Sales Contacts for Product-Le
- [[meta/import-error-systematic-resolution]] — Systematic ImportError Resolution — Domain Skill
- [[meta/lateral-reading-and-source-credibility]] — Lateral Reading and Source Credibility Verification
- [[meta/lead-generation-and-enrichment-for-regional-payment-service-providers-psps-8a075]] — **** Lead Generation and Enrichment for Regional Payment Service Providers (PSPs)
- [[meta/learning-queue]] — Learning Queue
- [[meta/llm-capability-detection]] — Models known to NOT support tool calling (as of 2026)
- [[meta/missing-import-detection-and-resolution]] — Missing Import Detection and Resolution
- [[meta/model-tool-support-and-capability-detection]] — Model Tool Support and Capability Detection
- [[meta/openrouter-credit-management-and-cost-optimization]] — OpenRouter Credit Management & Cost Optimization
- [[meta/query-decomposition-and-multi-hop-research]] — Query Decomposition and Multi-Hop Research Strategy
- [[meta/rapid-ecological-data-visualization-with-python-6b918e6d]] — **** Rapid Ecological Data Visualization with Python
- [[meta/reliable-weather-forecast-retrieval-78f4c768]] — **** Reliable Weather Forecast Retrieval
- [[meta/response-synthesis-optimization]] — Response Synthesis Optimization
- [[meta/robust-weather-data-retrieval-via-open-meteo-api-f39c9571]] — **** Robust Weather Data Retrieval via Open-Meteo API
- [[meta/selecting-payment-service-providers-psps-for-plg-platforms-in-cee-ticketi-5fe6eb]] — **** Selecting Payment Service Providers (PSPs) for PLG platforms in CEE ticketing/live events.
- [[meta/self-state-contextual-awareness-635ef455]] — self-state contextual awareness
- [[meta/signal-file-attachment-automation-macos-d2b6d66d]] — signal_file_attachment_automation_macos
- [[meta/skill-case-insensitive-lookup-with-original-casing-preservation-03148edd]] — Skill: Case-Insensitive Lookup with Original Casing Preservation
- [[meta/skill-distinguishing-load-balancers-from-reverse-proxies-c54a733a]] — Skill: Distinguishing Load Balancers from Reverse Proxies
- [[meta/skill-docker-containerization-basics-aca85774]] — SKILL: Docker Containerization Basics
- [[meta/skill-global-forest-cover-monitoring-data-retrieval-285935f5]] — **Skill: Global Forest Cover Monitoring & Data Retrieval**
- [[meta/skill-high-urgency-email-triage-63262acf]] — **Skill: High-Urgency Email Triage**
- [[meta/skill-selecting-payment-service-providers-psps-for-cee-baltic-ticketing-eb543bc0]] — **Skill: Selecting Payment Service Providers (PSPs) for CEE/Baltic Ticketing**
- [[meta/skill-two-entity-operational-model-a5f712f8]] — **SKILL: Two-Entity Operational Model**
- [[meta/skill-validating-file-existence-before-automation-tasks-6bc96b93]] — SKILL: Validating File Existence Before Automation Tasks
- [[meta/skill-validating-file-existence-before-automation-tasks-c944503e]] — SKILL: Validating File Existence Before Automation Tasks
- [[meta/skill-weather-forecast-policy]] — Skill: Weather Forecast Policy (Anti-Refusal Loop)
- [[meta/structured-coding-problem-solving-pedac]] — Structured Coding Problem Solving (PEDAC Method)
- [[meta/structured-research-to-identify-verified-sales-contacts-for-regional-paymen-a3ed]] — **** Structured research to identify verified sales contacts for regional payment service providers.
- [[meta/structured-research-to-identify-verified-sales-contacts-individuals-email-a92038]] — **** Structured research to identify verified sales contacts (individuals, emails, LinkedIn) for reg
- [[meta/systematic-debugging-methodology]] — Systematic Debugging Methodology for Coding Tasks
- [[meta/time-aware-4290a73b]] — Time-aware
- [[meta/time-based-email-counting-precision-e90ecab4]] — Time-based Email Counting Precision
- [[meta/topic-ca68c901]] — **Topic**
- [[meta/weather-forecast-policy-anti-refusal-loop-ccd91414]] — **** Weather Forecast Policy (Anti-Refusal Loop)
- [[meta/weather-retrieval-reliability-strategies-5fb26a75]] — Weather_retrieval_reliability_strategies
- [[meta/web-scraping-using-firecrawl-ad705cb6]] — Web scraping using Firecrawl
- [[meta/white-paper-concise-summary-methodology-134a9a00]] — White paper concise summary methodology

## Self (Agent Self-Knowledge)
- [[self/capability_inventory]] — capability_inventory
- [[self/inquiry_questions]] — inquiry_questions
- [[self/shadow-analysis]] — Shadow analysis

## Philosophy (Compiled Philosophical Frameworks)
(No pages yet.)

## PLG
(No pages yet.)

## Archibal
(No pages yet.)

## KaiCart
(No pages yet.)
